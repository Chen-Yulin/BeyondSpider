...
- `ShipCore.xml`、`Captain.xml`、`SuperCapacitor.xml`、`RadarPanel.xml`、`DefenseDirector.xml`、`ShieldProjector.xml`、`CiwsBlock.xml`、`HeavyNuclearMissile.xml`、`SpaceFlakTurret.xml`、`Railgun.xml`、`LaserLance.xml`、`SpaceInterceptorLauncher.xml`、`SpaceGunner.xml`：当前已注册零件。
