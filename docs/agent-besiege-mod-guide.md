...
- ~~`ShipPoseNet.cs`：舰船位置全精度联机复制（ADR-0021）~~ **已废弃/撤销**。设计是发 per-cluster-base 全精度位姿 + `pollTransform=false` 去重 + **反射写引擎私有 `NetworkBlock.transformMatrix`**。实机加载直接被 **Besiege 模组安全校验拒绝**：`[Security] You are not allowed to use System.Reflection.FieldInfo ... Assembly did not validate, not loading`——**Besiege 加载器禁止运行时 `System.Reflection`（也禁 Harmony），一个违规符号会让整个 DLL 不加载、所有功能全没**。这正是本 mod 从来不用反射的原因（是强制的、不是风格）。已删掉 `ShipPoseNet.cs`。远距离精度就靠 ADR-0019 的压缩盒扩大（20km 下约 0.15m）顶着。**给后续 agent 的硬约束：mod 运行时代码里绝不能出现 `System.Reflection` / Harmony。** 详见 ADR-0021。
